...
import { AstroInputTypesEnum, AttendanceStatus, DateFormat, EMPLOYEE_STATUS, getAttendanceStatusColor, getStatus, InputRegex, LEAVE_TYPE_LIST, PAYMENT_STATUS, STATUS_COLORS, TASK_LIST, TimeFormat } from '../../config/commonVariable';
import { RiUserReceivedLine } from 'react-icons/ri';
import { Controller, useFieldArray, useForm } from 'react-hook-form';
import { DatePicker, ConfigProvider, Select, Space } from 'antd';
                        ...
                        <div className="row g-2">
                            ...
                            <div className="col-12 col-md-6 col-lg-2 d-flex flex-column">
                                ...
                                <button
                                    ...
                                    style={{ height: '40px' }}
                                    onClick={() => navigat(PATHS.ADD_ASSIGN_TASK)}
                                >
                        ...
                        <div className="table-responsive">
                            <DataTable
                                ...
                                {/* <Column field="status" sortable data-pc-section="root" header="Status" style={{ minWidth: '8rem' }} body={(rowData) => (
                                    <>
                                        <span
                                            className={`p-tag p-component badge  text-light fw-semibold px-2 rounded-4 py-1 status_font cursor_pointer ${getAttendanceStatusColor(rowData?.status) || "bg-secondary"}`}
                                            data-pc-name="tag"
                                            data-pc-section="root"
                                            onClick={() => {
                                                ...
                                                {editing ? (
                                                    <Select
                                                        ...
                                                        onChange={(selectedId) => {
                                                            ...
                                                            setEditing(false); // close dropdown after select
                                                            console.log("Selected status:", selectedId, "for row:", rowData);

                                                            // if you want to save instantly to backend:
                                                            // updateStatus(rowData.id, selectedId);

                                                            // or with react-hook-form:
                                                            // setValue(AstroInputTypesEnum.STATUS, selectedId);
                                                        }}
                                                        options={
                                                            TASK_LIST?.map((task) => ({
                                                                label: task.name,
                                                        ...
                                                        autoFocus
                                                        onBlur={() => setEditing(false)} // if clicked outside
                                                    />
                                                ) : (
                                                    <Column field="status" sortable data-pc-section="root" header="Status" style={{ minWidth: '8rem' }} body={(rowData) => (
                                                        <>
                                ...
                                <Column field="status" header="Action" style={{ minWidth: '6rem' }} body={(rowData) => (
                                    <div className="action-btn">
                                        ...
                                        <a className="text-info edit cursor_pointer cursor_pointer me-1" onClick={() => navigat(PATHS?.EDIT_ASSIGN_TASK, { state: rowData })} >
